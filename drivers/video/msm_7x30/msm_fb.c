...
static void *virt_addr[3] = { NULL };
static void *phys_addr[3] = { NULL };
...
char *get_fb_addr(void)
{
	...
	if (!usb_pjt_info.latest_offset) {
		printk(KERN_ERR "%s: wrong address sent via ioctl?\n", __func__);
		return 0;
	}
			...
			if (mapped<3) {
				for (i=0; i<3; i++) {
					...
					if (!phys_addr[i]) {
						...
						get_pmem_file((unsigned int)phys_addr[i], &pmem_base, &pmem_vbase, &pmem_size, NULL);
